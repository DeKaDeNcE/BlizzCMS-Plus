	...
	<div class="uk-container">
			...
			<div class="uk-card-header">
				<div class="uk-grid uk-grid-small" data-uk-grid>
					<div class="uk-width-expand">
						<h4 class="uk-h4 uk-text-bold"><span
								uk-icon="icon: comments; ratio: 1.2"></span>&nbsp;<?= $this->forum_model->getSpecifyPostName($idlink); ?>
						</h4>
					</div>
							...
							<?php if ($this->forum_model->getSpecifyPostAuthor($idlink) == $this->session->userdata('wow_sess_id')): ?>
								<div class="uk-text-center uk-text-right@s">
									<a href="#" class="uk-button uk-button-default uk-button-small"
									   uk-toggle="target: #editTopic"><i
											class="far fa-edit"></i> <?= $this->lang->line('button_edit_topic'); ?></a>
								</div>
							<?php endif; ?>
						<?php endif; ?>
					</div>
			...
			<div class="uk-card-body">
				<div class="uk-grid uk-grid-small" data-uk-grid>
					<div class="uk-width-1-6@s">
						<div
							class="Author <?php if ($this->wowauth->getRank($this->forum_model->getSpecifyPostAuthor($idlink)) > 0) echo 'topic-author-staff'; ?> uk-flex uk-flex-center">
							<div class="topic-author-avatar profile">
								<?php if ($this->wowgeneral->getUserInfoGeneral($this->forum_model->getSpecifyPostAuthor($idlink))->num_rows()): ?>
									<img
										src="<?= base_url('assets/images/profiles/') . $this->wowauth->getNameAvatar($this->wowauth->getImageProfile($this->forum_model->getSpecifyPostAuthor($idlink))); ?>"
										alt=""/>
								<?php else: ?>
		...
		<div class="uk-grid uk-grid-small uk-child-width-1-1 uk-margin" data-uk-grid>
				...
				<div>
					<div class="uk-card uk-card-default uk-card-body">
						<div class="uk-grid uk-grid-small" data-uk-grid>
							<div class="uk-width-1-6@s">
								<div
									class="Author <?php if ($this->wowauth->getRank($commentss->author) > 0) echo 'topic-author-staff'; ?> uk-flex uk-flex-center">
									<div class="topic-author-avatar profile">
										<?php if ($this->wowgeneral->getUserInfoGeneral($commentss->author)->num_rows()): ?>
											<img
												src="<?= base_url('assets/images/profiles/' . $this->wowauth->getNameAvatar($this->wowauth->getImageProfile($commentss->author))); ?>"
												alt=""/>
										<?php else: ?>
											<img src="<?= base_url('assets/images/profiles/default.png'); ?>" alt=""/>
										<?php endif; ?>
								...
								<?php if ($this->wowauth->getRank($this->session->userdata('wow_sess_id')) > 0 || $this->session->userdata('wow_sess_id') == $commentss->author && $this->wowgeneral->getTimestamp() < strtotime('+30 minutes', $commentss->date)): ?>
									<div class="uk-margin-small-top uk-margin-remove-bottom">
										<button class="uk-button uk-button-danger uk-button-small"
												value="<?= $commentss->id ?>" id="button_delete<?= $commentss->id ?>"
												onclick="DeleteTopicReply(event, this.value)"><i
												class="fas fa-eraser"></i> <?= $this->lang->line('button_remove'); ?>
										</button>
									</div>
								<?php endif; ?>
							</div>
			...
			<?php if (!$this->wowauth->isLogged() && $this->forum_model->getTopicLocked($idlink) == 0): ?>
				<div>
					<div class="uk-card uk-card-default uk-card-body">
						<h3 class="uk-h3 uk-text-center"><span
								uk-icon="icon: comment; ratio: 1.5"></span> <?= $this->lang->line('forum_comment_header'); ?>
						</h3>
						<div class="glass-box-container">
							<p class="uk-margin-small"><?= $this->lang->line('forum_comment_locked'); ?></p>
							<a href="<?= base_url('login'); ?>"
							   class="uk-button uk-button-default uk-width-1-2 uk-width-1-3@m"><i
									class="fas fa-sign-in-alt"></i> <?= $this->lang->line('button_login'); ?></a>
						</div>
					</div>
				</div>
			<?php endif; ?>
			<?php if ($this->forum_model->getTopicLocked($idlink) == 1): ?>
				<div>
					<div class="uk-card uk-card-default uk-card-body">
						<h3 class="uk-h3 uk-text-center"><span
								uk-icon="icon: lock; ratio: 1.5"></span> <?= $this->lang->line('forum_not_authorized'); ?>
						</h3>
						<div class="glass-box-container">
							...
							<div class="uk-width-2-3@s">
								...
								<div class="uk-margin-small">
									<button class="uk-button uk-button-default uk-width-1-1" type="submit"
											id="button_reply"><i
											class="fas fa-reply"></i> <?= $this->lang->line('button_add_reply'); ?>
									</button>
								</div>
								<?= form_close(); ?>
							</div>
