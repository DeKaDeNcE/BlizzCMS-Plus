			...
			<div class="uk-grid uk-grid-small" data-uk-grid>
				<div class="uk-width-expand">
					<h4 class="uk-h4 uk-text-uppercase uk-text-bold"><i
							class="fas fa-list"></i> <?= $this->forum_model->getCategoryName($idlink); ?></h4>
				</div>
				<div class="uk-width-auto">
					<?php if ($this->wowauth->isLogged()): ?>
						<div class="uk-text-center uk-text-right@s">
							<a href="<?= base_url('forum/topic/new/' . $idlink); ?>"
							   class="uk-button uk-button-default uk-button-small"><i
									class="fas fa-pencil-alt"></i> <?= $this->lang->line('button_new_topic'); ?></a>
						</div>
					<?php endif; ?>
				</div>
			</div>
				...
				<?php foreach ($this->forum_model->getSpecifyCategoryPostsPined($idlink)->result() as $lists): ?>
					<tr>
						...
						<td class="uk-width-small uk-text-center">
							<span
								uk-icon="icon: commenting; ratio: 0.9"></span>&nbsp;<?= $this->forum_model->getComments($lists->id)->num_rows(); ?>
						</td>
						<td class="uk-width-small uk-text-center">
							<?php if ($this->wowauth->getRank($lists->author) > 0): ?>
								<span
									class="topic-forum-staff"><?= $this->wowauth->getUsernameID($lists->author); ?></span>
							<?php else: ?>
								<span
									class="topic-forum-member"><?= $this->wowauth->getUsernameID($lists->author); ?></span>
							<?php endif; ?>
						</td>
