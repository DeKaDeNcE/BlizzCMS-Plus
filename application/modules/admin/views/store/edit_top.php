	...
	<div class="uk-container">
		<div class="uk-grid uk-grid-small uk-margin-small" data-uk-grid>
			...
			<div class="uk-width-auto">
				<a href="<?= base_url('admin/store/top'); ?>" class="uk-icon-button"><i
		...
		<div class="uk-card uk-card-default">
			<div class="uk-card-body">
					...
					<div class="uk-form-controls">
						<select class="uk-select" id="store_top_item">
							...
							<?php foreach ($this->admin_model->getStoreItems() as $item): ?>
								<option
									value="<?= $item->id ?>"<?php if ($this->admin_model->getTopSpecifyItem($idlink) == $item->id) echo 'selected'; ?>><?= $item->name ?>
									(<?= $this->admin_model->getStoreCategoryName($item->category); ?>
									- <?= $this->wowrealm->getRealmName($this->admin_model->getStoreCategoryRealm($item->category)); ?>
									)
								</option>
							<?php endforeach; ?>
						</select>
				...
				<div class="uk-margin-small">
					<button class="uk-button uk-button-primary uk-width-1-1" type="submit" id="button_uptop"><i
