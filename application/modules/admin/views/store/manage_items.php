	...
	<div class="uk-container">
		<div class="uk-grid uk-grid-small uk-margin-small" data-uk-grid>
			<div class="uk-width-expand uk-heading-line">
				<h3 class="uk-h3"><i
						class="fas fa-shopping-cart"></i> <?= $this->lang->line('admin_nav_manage_store'); ?></h3>
			</div>
			...
			<div class="uk-width-1-4@s">
				<div class="uk-card uk-card-secondary">
					<ul class="uk-nav uk-nav-default">
						<li><a href="<?= base_url('admin/store'); ?>"><i
									class="fas fa-tags"></i> <?= $this->lang->line('section_store_categories'); ?></a>
						</li>
						<li class="uk-active"><a href="<?= base_url('admin/store/items'); ?>"><i
									class="fas fa-boxes"></i> <?= $this->lang->line('section_store_items'); ?></a></li>
						<li><a href="<?= base_url('admin/store/top'); ?>"><i
									class="fas fa-parachute-box"></i> <?= $this->lang->line('section_store_top'); ?></a>
						</li>
					</ul>
				</div>
			</div>
						...
						<table class="uk-table uk-table-middle uk-table-divider uk-table-small">
							...
							<?php if (isset($storeitemList) && !empty($storeitemList)): ?>
								<?php foreach ($storeitemList as $item): ?>
									<tr>
										<td><?= $item->name ?></td>
										<td><?= $this->admin_model->getStoreCategoryName($item->category); ?>
											- <?= $this->wowrealm->getRealmName($this->admin_model->getStoreCategoryRealm($item->category)); ?></td>
										...
										<td>
											<div class="uk-flex uk-flex-left uk-flex-center@m uk-margin-small">
												<a href="<?= base_url('admin/store/item/edit/' . $item->id); ?>"
												   class="uk-button uk-button-primary uk-margin-small-right"><i
														class="fas fa-edit"></i></a>
												<button class="uk-button uk-button-danger" value="<?= $item->id ?>"
														id="button_delete<?= $item->id ?>"
														onclick="DeleteItem(event, this.value)"><i
