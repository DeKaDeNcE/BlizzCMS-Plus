		...
		<div class="uk-grid uk-grid-medium" data-uk-grid>
						...
						<div class="uk-grid uk-grid-small" data-uk-grid>
							<div class="uk-width-expand@s">
								<h5 class="uk-h5 uk-text-bold"><i
										class="fas fa-newspaper"></i> <?= $this->news_model->getNewTitle($idlink); ?>
								</h5>
							</div>
							<div class="uk-width-auto@s">
								<p class="uk-text-small"><i
										class="far fa-clock"></i> <?= date('F j, Y, h:i a', $this->news_model->getNewlogDate($idlink)); ?>
								</p>
							</div>
						</div>
					</div>
											...
											<div class="topic-author-avatar profile">
												<?php if ($this->wowgeneral->getUserInfoGeneral($commentss->author)->num_rows()): ?>
													<img
														...
														alt=""/>
												<?php else: ?>
													<img src="<?= base_url('assets/images/profiles/default.png'); ?>"
														 alt=""/>
												<?php endif; ?>
											</div>
										</div>
										...
										<?php if ($this->wowauth->getRank($this->session->userdata('wow_sess_id')) > 0 || $this->session->userdata('wow_sess_id') == $commentss->author && $this->wowgeneral->getTimestamp() < strtotime('+30 minutes', $commentss->date)): ?>
											<div class="uk-margin-small-top">
												<button class="uk-button uk-button-danger uk-button-small"
														value="<?= $commentss->id ?>"
														id="button_delete<?= $commentss->id ?>"
														onclick="DeleteReply(event, this.value)"><i
			...
			<div class="uk-width-1-4@m">
				<div class="uk-card uk-card-default">
					<div class="uk-card-header">
						<h5 class="uk-h5 uk-text-bold"><i
								class="fas fa-list-alt"></i> <?= $this->lang->line('home_latest_news'); ?></h5>
					</div>
					<div class="uk-card-body">
						<ul class="uk-list uk-list-divider uk-text-small">
							<?php foreach ($this->news_model->getExtendedNewsList()->result() as $list): ?>
								<li>
									<a href="<?= base_url('news/' . $list->id) ?>"><i
											class="far fa-newspaper"></i> <?= $list->title ?></a>
								</li>
							<?php endforeach; ?>
						</ul>
					</div>
